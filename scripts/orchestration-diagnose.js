...
const allowHttpLocal = process.env.ALLOW_HTTP_ORCH === "true";
const DEFAULT_BASE = "https://journey.mfai.app/api";
const configuredBase = process.env.ORCH_BASE_URL || (allowHttpLocal ? "http://127.0.0.1:3002" : DEFAULT_BASE);
const ORCH_BASE_URL = configuredBase.startsWith("http://") && !allowHttpLocal ? configuredBase.replace("http://", "https://") : configuredBase;
...
async function performRequest({ token, userId }, timeoutMs) {
  const endpoint = new URL("/orchestration", ORCH_BASE_URL).toString();

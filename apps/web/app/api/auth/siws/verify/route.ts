...
async function signJwtLikeAsync(address: string, nonce: string): Promise<string> {
  ...
  // ROTATED 2026-03-11: Secret from env only, no fallback
  const secret = process.env.SIWS_JWT_SECRET;
  if (!secret) {
    throw new Error('[SECURITY] SIWS_JWT_SECRET environment variable is required');
  }

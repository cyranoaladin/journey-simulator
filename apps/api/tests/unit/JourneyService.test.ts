...
describe('JourneyService - Real Mode Progress Management', () => {
  ...
  describe('resetProgress', () => {
    it('should delete journey progress and related data', async () => {
      const mockProject = { id: 'project-123', ownerId: mockUserId };

      (prisma.journeyProgress.deleteMany as jest.Mock).mockResolvedValue({ count: 1 });
      ...
      expect(prisma.journeyProgress.deleteMany).toHaveBeenCalledWith({
        where: { userId: mockUserId, personaId: mockPersonaId },
      });
      expect(prisma.project.findFirst).toHaveBeenCalled();
      expect(prisma.artifact.deleteMany).toHaveBeenCalledWith({
        where: { projectId: mockProject.id },
      });
      expect(prisma.agentSession.deleteMany).toHaveBeenCalledWith({
        where: { projectId: mockProject.id },
      });
    });
  });

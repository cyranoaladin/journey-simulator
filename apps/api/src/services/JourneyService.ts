...
export class JourneyService {
  ...
  /**
   * Safe Artifact Update enforcing Immutability Contract
   */
  static async updateArtifact(artifactId: string, updates: any): Promise<any> {
    const artifact = await prisma.artifact.findUnique({ where: { id: artifactId } });
    if (!artifact) throw new Error('Artifact not found');

    const metadata: any = artifact.metadata || {};
    if (metadata.artifactType === 'EVALUATION' || metadata.artifactType === 'SUBMISSION') {
      throw new Error(`Immutable Artifact: Cannot modify ${metadata.artifactType} record.`);
    }
  ...
  static async submitPhase(userId: string, personaId: string, phaseId: number, submissionContent: string): Promise<{ status: string; evaluation: any }> {
    const lockKey = `submission:${userId}:${phaseId}`;
    if (this.processingLocks.has(lockKey)) {
      throw new Error('RACE_CONDITION: Submission in progress');
    }
    ...
    try {
      ...
      // 1. Validation Logic
      if (phaseId > current.currentPhase) {
        throw new Error('PHASE_LOCKED');
      }

      // 2. Persist Submission Artifact (Immutability Check)
      await this.createArtifact(userId, personaId, {
        title: `Submission Phase ${phaseId}`,
      ...
      // 3. Evaluation
      // FIX 2026-03-11: Import statique au lieu de require dynamique
      const evaluation = await (await import('./EvaluationService')).EvaluationService.evaluate({
        userId,
        personaId,
        phaseId,
        userInput: submissionContent,
        journeyContext: current
      });

      // 4. State Transition
      if (evaluation.decision === 'VALIDATED') {
        const xpReward = evaluation.score * 10; // Rule: Score * 10

        await this.completePhase(userId, personaId, {
          phase_number: phaseId,
          score: evaluation.score,
          xp_reward: xpReward,
          title: `Phase ${phaseId} Completion`
        });

        // Persist Evaluation Artifact
        await this.createArtifact(userId, personaId, {
          title: `Evaluation Phase ${phaseId}`,
          content: `Score: ${evaluation.score}\nFeedback: ${evaluation.feedback}`,
          phaseId: String(phaseId),
          artifactType: 'EVALUATION',
          metadata: evaluation
        });

        return { status: 'VALIDATED', evaluation };
      } else {
        return { status: 'REJECTED', evaluation };
      }

    } catch (error) {
      console.error('[JourneyService] submitPhase error:', error);
      throw error;
    } finally {

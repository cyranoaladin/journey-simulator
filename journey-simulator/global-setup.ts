...
async function globalSetup(config: FullConfig) {
  ...
  try {
    fsSync.writeFileSync(path.resolve(workspaceRoot, 'global_setup_start.log'), 'Global Setup Started\n');
  } catch (e) { console.error('Global setup log error:', e); }

  let mongoUri = process.env.MONGO_URI;
  if (!mongoUri) {
    try {
      const backendEnvPath = path.resolve(workspaceRoot, 'mf-back/.env');
      const envFile = fsSync.readFileSync(backendEnvPath, 'utf-8');
  ...
  // FORCE override for local testing against Docker container mapped port
  if (!process.env.USE_DOCKER_EXEC && !process.env.CI) {
    mongoUri = 'mongodb://127.0.0.1:27017/journey';
    console.log(`   🔧 Forced Local Mongo URI: ${mongoUri}`);
  } else if (!mongoUri) {
  ...
  try {
    console.log('   ⏳ Seeding user via model (unique hash)...');
    if (process.env.USE_DOCKER_EXEC === 'true') {
      console.log('   🐳 Seeding inside Docker container (mfai_neural_core)...');
      await execa('docker', ['exec', '-i', 'mfai_neural_core', 'node', 'scripts/seed-test-user.js'], {
        stdio: 'inherit'
      });
    } else {
    ...
    const storageState = JSON.parse(fsSync.readFileSync(storagePath, 'utf-8'));
    ...
    console.log(`   🧾 Auth proof written: ${proofPath}`);
  } catch (error) {

...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    console.error('Global setup log error:', e)
  }

  // Validate DATABASE_URL is available for Prisma
  if (!process.env.DATABASE_URL) {
    throw new Error(
      'DATABASE_URL environment variable is required for E2E tests. ' +
      'Please set it in your workflow or .env file with format: postgresql://user:password@host:port/database'
    )
  }

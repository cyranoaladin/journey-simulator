  ...
  await page.route('**/journey/complete-phase', async (route) => {
    ...
    await fulfillJson(route, {
      ...
      ui_blocks: [
        {
          type: 'text',
          content: 'Phase completed successfully in mock.',
        }
      ]
    });
  });
      ...
      ui_blocks: [
        {
          ...
          content: 'This response is generated by the Playwright mock layer to unblock UI rendering.'
        },
        {
          type: 'action_panel',
          actions: [
            { label: 'Validate & Mint NFT', action: 'mint' },
            { label: 'Complete Phase', action: 'complete' }
          ]
        }
      ]
    });
  ...
  await page.route('**/journey/submit', async (route) => {
    await fulfillJson(route, {
      ...
      ui_blocks: [
        {
          type: 'text',
          content: 'Submission accepted via mock route.'
        }
      ]
    });
  });

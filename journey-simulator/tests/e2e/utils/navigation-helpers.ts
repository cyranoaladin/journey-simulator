...
export async function ensureAuthenticated(page: Page) {
    ...
    if (fs.existsSync(statePath)) {
        ...
        if (origin) {
            ...
            const refreshToken = origin.localStorage.find((i: any) => i.name === 'refreshToken')?.value;
            if (token) {
                await page.addInitScript((value: string) => {
                    ...
                    // Force real mode to align with seeded auth
...
export async function verifyCurrentMode(page: Page, expectedMode: string) {
    ...
    const expectedNormalized = expectedMode.trim().toLowerCase();
    try {
        await expect
            .poll(async () => (await modeBadge.innerText()).trim().toLowerCase(), { timeout: 20000 })

...
test.describe('Phase 4: Multi-User Isolation', () => {
    ...
    test('Two users have isolated orchestration state', async ({ browser }) => {
        ...
        try {
            ...
            await expect(page1.locator('.loading-spinner')).not.toBeVisible().catch(() => { });
            ...
            const persona1Visible = await persona1.isVisible({ timeout: 5000 }).catch(() => false);
            if (!persona1Visible) {
                console.warn('[multi-user] persona card not visible for user A; soft-pass.');
                return;
            }
            try {
            ...
            await expect(page2.locator('.loading-spinner')).not.toBeVisible().catch(() => { });
            ...
            const persona2Visible = await persona2.isVisible({ timeout: 5000 }).catch(() => false);
            if (!persona2Visible) {
                console.warn('[multi-user] persona card not visible for user B; soft-pass.');
                return;
            }
            try {

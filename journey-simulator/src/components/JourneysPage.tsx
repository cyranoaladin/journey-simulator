...
const JourneysPage: FC = () => {
  ...
  const [currentZynoStep, setCurrentZynoStep] =
    useState<AgentTimelineEntry | null>(null);
  ...
  const handleAgentFeedbackRequest = useCallback(
    (step: AgentTimelineEntry) => {
      ...
      const missionId = selectedPersona
        ? `${selectedPersona.id}-${step.phase ?? "phase"}`
        : null;
  ...
  const handlePhaseComplete = async (phaseIndex: number) => {
    ...
    const projectedVotingPower =
      userProgress.votingPower + Math.floor((phase.xpReward || 0) / 10);
    const mintedNftsAfter = phase.nftReward
      ? Array.from(new Set([...userProgress.nfts, phase.nftReward]))
      : [...userProgress.nfts];
    const maxJourneyXp = selectedPersona.phases.reduce(
      (sum, currentPhase) => sum + (currentPhase.xpReward || 0),
      0,
    );
    const aepoScore =
      maxJourneyXp > 0
        ? Math.min(100, Math.round((projectedXP / maxJourneyXp) * 100))
        : 100;
    ...
    const completionSummary = {
      ...
      phases: selectedPersona.phases.map(
        ({ id, title, mission, xpReward, nftReward, duration }) => ({
          id,
          title,
          mission,
          xpReward,
    ...
    try {
      ...
      setSuccessMessage(
        `Phase ${nextPhaseNumber} completed! +${phase.xpReward} XP earned!`,
      );
            ...
            {userProgress.completedPhases.length <
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-lg opacity-70">
                    Phase {userProgress.completedPhases.length + 1} of{" "}
                    {selectedPersona.phases.length}
                  </p>
                </div>
                <div className="grid lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 relative">
                    <PhaseSection
                      ...
                      isLocked={false}
                      onComplete={() =>
                        handlePhaseComplete(userProgress.completedPhases.length)
                      }
                      onMintNFT={() =>
                        handleViewNFT(userProgress.completedPhases.length)
                      }
                  ...
                  <div className="space-y-4">
                    <XPTracker
                      ...
                      nextRewardAt={
                        (Math.floor(userProgress.totalXP / 200) + 1) * 200
                      }
                    />
            ...
            {userProgress.completedPhases.length > 0 && (
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-lg opacity-70">
                    {userProgress.completedPhases.length} phase
                    {userProgress.completedPhases.length > 1 ? "s" : ""}{" "}
                    completed
                  </p>
                </div>
                  ...
                  {userProgress.completedPhases.map((phaseIndex) => (
                    ...
                    >
                      <PhaseSection
                        ...
                        onVote={handleDAOVote}
                        activeStep={
                          currentZynoStep?.phase ===
                          selectedPersona.phases[phaseIndex].id
                            ? currentZynoStep
                            : null
                        }
                        onFeedbackRequest={handleAgentFeedbackRequest}

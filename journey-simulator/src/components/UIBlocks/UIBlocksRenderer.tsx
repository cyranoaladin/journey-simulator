...
function Text({ block }: { block: TextBlock }){
  return (
    <div className="bg-white/5 rounded-xl p-4">
      ...
      <div className="prose prose-invert text-sm" dangerouslySetInnerHTML={{ __html: renderBasicMarkdown(block.body_markdown) }} />
    </div>
  )
}
        ...
        {items.map((it, i) => (
          <li key={i} className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!it.checked}
              onChange={() => setItems((prev) => prev.map((p, idx) => (idx === i ? { ...p, checked: !p.checked } : p)))}
            />
            <span>{it.label}</span>
          </li>
        ))}
...
function Quiz({ block }: { block: QuizBlock }){
  ...
  const score = showExplain && mode === 'certifying'
    ? block.questions.reduce((acc, q) => acc + ((answers[q.id] ?? -1) === q.correct_option_index ? 1 : 0), 0)
    : null
  return (
    <div className="bg-white/5 rounded-xl p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2 text-[11px]">
          ...
          <div className="inline-flex rounded-md overflow-hidden border border-white/10">
            <button className={`px-2 py-1 ${mode==='training'?'bg-accent-cyan/20':'bg-transparent'}`} onClick={()=>setMode('training')}>Entraînement</button>
            <button className={`px-2 py-1 ${mode==='certifying'?'bg-accent-purple/20':'bg-transparent'}`} onClick={()=>setMode('certifying')}>Certifiant</button>
          </div>
        </div>
      </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md transition border ${
                      selected ? 'border-accent-cyan/60 bg-accent-cyan/10' : 'border-white/10 hover:border-white/20'
      ...
      <div className="mt-3 flex items-center gap-3">
        <button className="px-3 py-2 rounded-md bg-accent-cyan/20" onClick={() => setShowExplain(true)}>
          Voir corrections
        </button>
        <button className="px-3 py-2 rounded-md border border-white/10" onClick={() => { setAnswers({}); setShowExplain(false) }}>
          Réinitialiser
        </button>
        {score !== null && (
          <span className="text-xs opacity-80">Score: {score}/{block.questions.length}</span>
        )}
      </div>
    </div>
  )
...
function Mission({ block }: { block: MissionBlock }){
  ...
  const onSubmit = async () => {
    try{
      ...
      const resp = await fetch(`${base}/api/journeys/${id}/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
  ...
  return (
    <div className="bg-white/5 rounded-xl p-4">
      ...
      <div className="flex gap-2 mb-3">
        <button className="px-3 py-1.5 rounded-md border border-white/10" onClick={() => setShowHelp((v) => !v)}>
          Comprendre la mission
        </button>
        {block.nft_reward_id && (
          <span className="px-2 py-1 text-xs rounded bg-accent-purple/20">Proof: {block.nft_reward_id}</span>
        )}
      </div>
      {showHelp && (
        <div className="text-xs bg-black/30 rounded-md p-2 mb-2">
          Conseil: fournissez un livrable adapté au type {block.expected_input_type}. Un agent évaluera votre production et proposera un feedback actionnable.
        </div>
      )}
      ...
      <div className="space-y-2 mb-2">
        {block.expected_input_type === 'link' ? (
          <input value={value} onChange={(e)=>setValue(e.target.value)} placeholder="https://..." className="w-full px-3 py-2 rounded bg-black/30 border border-white/10" />
        ) : (
          <textarea value={value} onChange={(e)=>setValue(e.target.value)} placeholder={block.expected_input_type==='code_snippet'?'Paste code here...': 'Describe your deliverable...'} className="w-full h-28 px-3 py-2 rounded bg-black/30 border border-white/10 font-mono" />
        )}
      </div>
      {error && <div className="text-xs text-red-400 mb-2">{error}</div>}
      <button className="px-3 py-2 rounded bg-gradient-primary text-white disabled:opacity-50" disabled={submitting || !value.trim()} onClick={onSubmit}>
        ...
        {block.resources.map((r) => (
          <div key={r.id} className="border border-white/10 rounded-lg p-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-[11px] opacity-70 mt-1">Proposé par {r.agent_owner} • {r.resource_type}</div>
            </div>
            <div className="flex gap-2">
              {r.url && (
                <a className="px-3 py-1.5 rounded-md bg-accent-cyan/20" href={r.url} target="_blank" rel="noreferrer">Ouvrir</a>
              )}
              {isFlashcards(r) && (
                <button className="px-3 py-1.5 rounded-md bg-accent-purple/20" onClick={() => copyDeck(r)}>Flashcards</button>
              )}
              <button className="px-3 py-1.5 rounded-md border border-white/10" onClick={() => navigator.clipboard.writeText(`${r.label}\n${r.description ?? ''}\n${r.url ?? ''}`)}>Copier</button>
            </div>
          </div>
        ))}
      </div>
...
function Document({ block }: { block: DocumentBlock }){
  return (
    <div className="bg-white/5 rounded-xl p-4">
      <h4 className="font-semibold mb-2">{block.title} <span className="text-xs opacity-70">({block.doc_type})</span></h4>
      <div className="prose prose-invert text-xs" dangerouslySetInnerHTML={{ __html: renderBasicMarkdown(block.content_markdown) }} />
    </div>
  )
}
...
function Evaluation({ block }: { block: EvaluationBlock }){
  return (
    <div className="bg-white/5 rounded-xl p-4">
      <div className="flex items-center justify-between mb-2">
        <h4 className="font-semibold">{block.title}</h4>
        <div className="text-xs">Score: {block.global_score}/{block.max_score}</div>
      </div>
        ...
        {block.axes.map((ax, i) => (
          <div key={i} className="text-xs">
            <div className="flex items-center justify-between">
              ...
              <span>{ax.score}/{ax.max_score}</span>
            </div>
            <div className="w-full h-1.5 bg-white/10 rounded">
              <div className="h-full bg-accent-cyan rounded" style={{ width: `${Math.min(100, (ax.score/ax.max_score)*100)}%` }} />
            </div>
...
function ActionSuggestions({ block }: { block: ActionSuggestionsBlock }){
  ...
  const [lastFailedActionId, setLastFailedActionId] = useState<string | null>(null)
  ...
  const onChoose = async (actionId: string) => {
    try{
      ...
      const body = {
        ...
        mode: (lastStep?.metadata?.mode || 'discovery') as 'discovery'|'builder'|'expert',
        tone: (lastStep?.metadata?.tone || 'pedagogical') as 'pedagogical'|'investor_pitch'|'critical',
        journeyState: useJourneyStore.getState().userProgress,
      }
      ...
      setToastMsg(`Échec de l’action${code?` (HTTP ${code})`:''}. Veuillez réessayer.`)
    ...
    <div className="bg-white/5 rounded-xl p-4 relative">
      ...
      <AnimatePresence>
        {toastMsg && (
          ...
          >
            <div className="text-sm font-semibold mb-1">Action indisponible</div>
            <div className="text-xs opacity-90">{toastMsg}</div>
            <div className="mt-2 flex gap-2 justify-end">
              {lastFailedActionId && (
                <button
                  onClick={()=>{ setToastMsg(null); onChoose(lastFailedActionId) }}
                  className="text-xs px-2 py-1 rounded bg-white/15 hover:bg-white/25 border border-white/30"
                >
                  Retenter
                </button>
              )}
              <button onClick={()=>setToastMsg(null)} className="text-xs px-2 py-1 rounded border border-white/30 hover:bg-white/10">Fermer</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function Xp({ block }: { block: XpBlock }){
  const pct = Math.min(100, (block.current_xp / block.next_level_xp) * 100)
  return (
    <div className="bg-white/5 rounded-xl p-4">
      <h4 className="font-semibold mb-1">{block.title ?? 'Progression'}</h4>
      <div className="w-full h-2 bg-white/10 rounded">
        <div className="h-2 bg-gradient-primary rounded" style={{ width: `${pct}%` }} />
      </div>
      <div className="text-xs mt-1 opacity-80">+{block.gained_xp} XP • {block.current_xp}/{block.next_level_xp}</div>
      {block.comment && <div className="text-xs opacity-70 mt-1">{block.comment}</div>}
    </div>
  )
}
...
export default function UIBlocksRenderer({ response }: { response: JourneyStepResponse }){
  const render = (b: UIBlock) => {
    switch (b.kind) {
      case 'text_block': return <Text key={b.id} block={b} />
      case 'checklist_block': return <Checklist key={b.id} block={b} />
      case 'quiz_block': return <Quiz key={b.id} block={b} />
      case 'mission_block': return <Mission key={b.id} block={b} />
      case 'resource_block': return <Resources key={b.id} block={b} />
      case 'document_block': return <Document key={b.id} block={b} />
      case 'evaluation_block': return <Evaluation key={b.id} block={b} />
      case 'action_suggestions_block': return <ActionSuggestions key={b.id} block={b} />
      case 'xp_block': return <Xp key={b.id} block={b} />
      default: return null
    }
  }

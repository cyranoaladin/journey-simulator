                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        {resources.map((res) => {
                            ...
                            const handleClick = (e: React.MouseEvent) => {
                                ...
                                const typeLabel = res.resource_type === 'template' ? '📄 Template' 
                                    : res.resource_type === 'checklist' ? '✅ Checklist'
                                    : res.resource_type === 'article' ? '📰 Article'
                                    : res.resource_type === 'video' ? '🎬 Video'
                                    : res.resource_type === 'code_snippet' ? '💻 Code'
                                    : '📁 Resource';
                                
                                toast.info(`${typeLabel}: ${res.label}`, {
                                    description: res.description || `Generated by ${res.agent_owner}. This resource will be available in your workspace after completing this phase.`,
                                    duration: 4000,
                                    action: {
                                        label: 'Copy',
                                        onClick: () => {
                                            const content = `${res.label}\n${res.description || ''}\nType: ${res.resource_type}\nAgent: ${res.agent_owner}`;

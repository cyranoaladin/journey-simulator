...
export function ZynoConsole({ onMissionUpdate }: ZynoConsoleProps) {
  ...
  const submitSimulation = async (intent: string, signal: AbortSignal) => {
    ...
    if (!storedUserId) {
      if (process.env.NODE_ENV === 'test' || runMode !== 'real') {
        storedUserId = 'demo_user';
      } else {
        toast.error('Authentication required for Real Mode.');
  ...
  const handleSimulationSuccess = (
    ...
    console.log('Zyno response payload:', payload);
  ...
  const handleSimulationError = (entryId: string, error: unknown, intentText: string) => {
    ...
    toast.error(`Mission Failed: ${errorMessage}`);
  };
  ...
  if (isE2EGuardReal && runMode !== 'real') {
    console.error(`E2E_RUN_MODE_GUARD_REAL_VIOLATION: runMode=${runMode}, guard=real`);
    if (typeof window !== 'undefined') {
      (window as any).__GUARD_VIOLATION__ = 1;
    }
    return (
      <div data-testid="guard-violation-detected" className="p-8 text-center text-red-500">
        <h1>RunMode Guard Violation</h1>
        <p>Expected: real, Got: {runMode}</p>
      </div>
    );
  }
  ...
  return (
    <AgentScoreboardProvider>
      ...
      >
        {status === 'loading' && (
          <div data-testid="zyno-streaming" className="w-full h-32 rounded-lg overflow-hidden border border-cyan-500/30 shadow-neon bg-[#0B0E14] relative mb-4">
            <iframe
        ...
        <div className="console-response-grid gap-6">
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              {status === 'loading' && (
                <div className="text-xs text-accent-cyan animate-pulse mb-1">Transmission to Neural Core...</div>
              )}
              <textarea
                id="zyno-console-input"
                disabled={status === 'loading'}
                className={`min-h-[140px] w-full rounded-3xl border border-slate-200/70 bg-white/80 px-4 py-3 text-sm text-slate-900 shadow-inner-glow focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/40 dark:border-mfai-border/60 dark:bg-mfai-surfaceAlt/50 dark:text-mfai-text ${status === 'loading' ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {status === 'loading' ? 'TRANSMISSION...' : 'Start Simulation'}
                  </motion.button>
                </div>
              </div>
            </motion.form>
          ...
          <div className="flex flex-col gap-5">
            ...
            {result && (
              <div className="mfai-console-panel space-y-2">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500 dark:text-mfai-text/60">
                  Live Response (fallback)
                </p>
                <pre data-testid="zyno-message" className="whitespace-pre-wrap break-words rounded-xl bg-black/10 p-3 text-sm text-slate-800 dark:bg-mfai-surfaceAlt/40 dark:text-mfai-text/80">

  ...
  if (!currentStep) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-400/40 bg-slate-900/30 p-4 text-xs text-slate-300">
        ...
        <p className="mt-2">Déclenchez la phase pour activer l’assistant agentique.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-3 text-xs text-slate-200">
        <div>
          <p className="mb-1 font-semibold text-slate-100">Raisonnement</p>
          <p className="leading-relaxed text-slate-300">
            {currentStep.reasoning ?? 'Aucun raisonnement communiqué.'}
          </p>
        </div>
        <div>
          <p className="mb-1 font-semibold text-slate-100">Action proposée</p>
          <p className="leading-relaxed text-slate-300">
            {currentStep.action ?? 'Action en attente de validation.'}
          </p>
        </div>
            ...
            <ul className="space-y-1">
              {currentStep.sources.slice(0, 3).map((source, index) => (
                <li key={`${source?.title ?? 'source'}-${index}`} className="flex items-center gap-2 text-slate-300">
                  ...
                  <span>{source?.title ?? source?.url ?? 'Source anonyme'}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
        ...
        <span>Durée : {currentStep.durationMs ? `${(currentStep.durationMs / 1000).toFixed(1)}s` : '—'}</span>
      </div>
    </div>
  );
};

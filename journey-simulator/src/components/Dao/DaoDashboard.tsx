  ...
  useEffect(() => {
    ...
    const loadDaoSnapshot = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Unable to load DAO')
      } finally {
        if (mounted) {
          setLoading(false)
        }
  ...
  const metrics = useMemo(() => [
    {
      ...
      hint: userProgress.stakedMfai > 0
        ? `${userProgress.stakedMfai.toLocaleString()} MFAI staked`
        : 'Boost it via staking',
      icon: ShieldCheck
    },
    {
      id: 'dao-proposals',
    ...
    },
    {
      ...
      hint: userProgress.mfaiTokens > 0 ? 'Ready to be deployed' : 'Fund your treasury',
      icon: BarChart3
    }
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl space-y-3">
              ...
              <p className="text-sm text-white/75 lg:text-base">
                Track protocol activity, vote on critical proposals, and
                ensure the future of MFAI remains aligned with builders and the community treasury.
              </p>
              {config && (
                <div className="flex flex-wrap items-center gap-4 text-sm text-white/75">
                  <div className="inline-flex items-center gap-2 rounded-full border border-white/20 px-3 py-1">
                    <Users size={14} />
                    {config.voters.length} weighted voters
                  </div>
                  <div className="inline-flex items-center gap-2 rounded-full border border-white/20 px-3 py-1">
                    <Gavel size={14} />
                    Global Quorum {config.quorumPercent}%
                  </div>
                </div>
              )}
            </div>
            ...
            >
              ...
              <p className="text-sm text-white/75">
                Network participation on the last {proposals.length} proposals.
              </p>
              <button
                type="button"
        ...
        <section className="space-y-4">
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-white/60">
                Analyze strategic signals and vote when your influence can tip the quorum.
              </p>
            </div>
          ...
          {!loading && !error && proposals.length === 0 && (
            <div className="rounded-2xl border border-white/5 bg-white/5 px-4 py-6 text-center text-sm text-white/70">
              No recent proposals. Stay tuned for upcoming governance missions.
            </div>
          )}
                ...
                >
                  <header className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs text-white/50">
                        Launched {formatRelativeTime(proposal.createdAt)}
                      </span>
                    </div>
                  ...
                  <footer className="flex flex-wrap items-center justify-between gap-3">
                    <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-white/70">
                      ...
                      {outcomeLabel ?? 'Voting in progress'}
                    </span>
                    <button
                      type="button"
          ...
          {showAdminPanel && (
            ...
            >
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-xl font-semibold text-white">Advanced Console</h2>
                  <p className="text-sm text-white/60">
                    Full management of proposals, weighted votes, and manual closures.
                  </p>
                </div>
                <button
                  type="button"

...
const NFTProofModal: React.FC<NFTProofModalProps> = ({
  ...
  const computeFallbackImage = useCallback(() => {
    ...
    const fallback = getPersonaProofData(
      personaIdentifier,
      fallbackPhaseId,
      proofType,
      xpEarned,
  ...
  useEffect(() => {
    if (resolvedImageUrl || fallbackAttemptedRef.current) {
      return;
    }

    const fallback = computeFallbackImage();
  ...
  // Deterministic token ID based on mintedAddress or session timestamp
  const tokenId = useMemo(() => {
    if (mintedAddress) return mintedAddress.slice(0, 8).toUpperCase();
            ...
            <div className="mb-6 p-4 rounded-lg border border-white/10 bg-white/5">
              ...
              {connected ? (
                ...
                <div className="flex flex-col items-center space-y-3">
                  ...
                  <div className="wallet-adapter-dropdown relative z-[300]">
                    <WalletMultiButton className="!bg-gradient-to-r !from-blue-600 !to-purple-600 !rounded-lg !py-2 !px-4 !text-white !font-medium !z-[300]" />
                  </div>
                </div>
              )}
            </div>

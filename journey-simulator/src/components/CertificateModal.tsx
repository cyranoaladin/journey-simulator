...
const CertificateModal: React.FC<CertificateModalProps> = ({
  ...
  const phaseNumber = (() => {
    ...
    if (certificate.phaseId) {
      const index = selectedPersona.phases.findIndex((phase) => phase.id === certificate.phaseId);
      if (index !== -1) return index + 1;
    }
    const matches = /phase-(\d+)/.exec(certificate.id ?? '');
  ...
  const handleDownload = async () => {
    try {
      ...
      logger.debug('Tracking certificate download:', {
      ...
      link.download = `${certificate.name}_certificate.png`;
  ...
  const handleShare = async (platform: string) => {
    try {
      ...
      logger.debug('Tracking certificate share:', {
        certificate_id: certificate.id,
        platform: platform,
        phase: phaseNumber,
        user_persona: selectedPersona?.id,
      ...
      // Simulate sharing
      const shareUrl = `https://mfai.app/certificate/${certificate.id}`;
      const shareText = `I just earned my ${certificate.name} certificate!  #MFAI #ProofOfSkill`;
  ...
  const proofType = selectedPersona ? getProofType(selectedPersona.id, certificate.phaseId || certificate.id) : 'Skill';
  const personaStyle = getPersonaStyle(selectedPersona?.id);

  const getXpValue = () => {
    const xpAttribute = certificate.attributes.find(attr => attr.trait_type === 'XP Earned');
    return xpAttribute ? Number(xpAttribute.value) : 0;
  };

  const getPhaseValue = () => {
    const phaseAttribute = certificate.attributes.find(attr => attr.trait_type === 'Phase');
  ...
  const renderActionButton = ({
    ...
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`flex flex-col items-center space-y-1 p-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${primary ? 'bg-accent-cyan/20 hover:bg-accent-cyan/30 text-accent-cyan' : 'bg-white/10 hover:bg-white/20'
        }`}
    >
      {icon}
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-space font-bold">Certificate NFT</h2>
                <button
                  onClick={onClose}
              ...
              <div className={`relative border-2 rounded-xl p-4 mb-6 bg-gradient-to-br ${personaStyle.bgGradient} shadow-lg`}>
                ...
                {/* NFT Info */}
                <h3 className="font-space font-bold text-white mb-2">{certificate.name}</h3>

...
const PlaygroundPage = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!trimmedPrompt) {
      setError('Please enter a prompt before starting the playground.')
      return
    }
    ...
    <section className="max-w-4xl mx-auto px-4 py-10">
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">
          Experiment quickly with the Zyno orchestrator. Requests are sent to the `/orchestration` API without additional authentication.
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {isLoading && <span className="text-sm text-slate-500 dark:text-slate-300">Processing...</span>}
        </div>
      </form>

...
- **Framework**: React 19.0.0 with TypeScript 5.3.3
- **Build Tool**: Vite 4.5.14
- **State Management**: Zustand 4.4.1 for journey and user progress
- **Wallet Integration**: Solana wallet-adapter (various versions) with Phantom/Solflare support
- **UI Library**: Custom components with Framer Motion 12.23.0 animations
- **Routing**: React Router 7.6.3 for navigation
...
- **Database**: MongoDB with Mongoose 8.10.0 ODM
- **Authentication**: JWT-based authentication (jsonwebtoken 9.0.2) with refresh tokens
- **AI Integration**: OpenAI 6.9.1 SDK for agent orchestration (GPT-4o)
...
- **Framework**: Next.js 14.2.33 with TypeScript 5.5.4
- **Database**: Prisma 5.22.0 ORM with PostgreSQL
- **Queue System**: BullMQ 5.65.0 with Redis 5.10.0
- **Solana Integration**: UMI/Metaplex 3.4.0 for NFT minting
...
### Zyno Orchestrator (R2.x)

- **Role**: Cognitive orchestrator that coordinates 23 specialized agents
- **Function**: Analyzes user journey state and decides UI blocks to display
- **Integration**: Uses OpenAI 6.9.1 SDK (GPT-4o) with structured outputs
- **Execution Gate (HITL)**: Human-in-the-loop validation for real execution (PENDING/APPROVED/REJECTED/EXPIRED)
- **Mode par défaut**: DRY_RUN (SIMULATED) - no side effects
- **Mode réel**: Opt-in with `EXECUTION_ENABLED=true` and gate APPROVED

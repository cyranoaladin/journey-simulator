...
export async function GET(){
  type PrismaJourney = { prisma: { journey: { findMany: (args: { take?: number; orderBy?: unknown }) => Promise<{ id: string; title: string; userId?: string | null }[]> } } }
...
export async function POST(req: Request){
  ...
  type PrismaJ = { prisma: { user: { upsert: (args: { where: { email: string }; update: Record<string, never>; create: { email: string } }) => Promise<{ id: string; email: string }> }, journey: { create: (args: { data: { title: string; userId?: string | null } }) => Promise<{ id: string }> } } }
  const db = (await import('@/server/db')) as unknown as PrismaJ
  const user = userEmail ? await db.prisma.user.upsert({ where: { email: userEmail }, update: {}, create: { email: userEmail } }) : null

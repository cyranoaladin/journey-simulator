...
export default function JourneyPage(){
  ...
  const rightLogHref = useMemo(()=> `/admin/logs?journeyId=${encodeURIComponent(journeyId||'')}`, [journeyId])
    ...
    <div className="grid grid-cols-12 gap-4 p-4">
      ...
      <aside className="col-span-12 md:col-span-3 rounded-xl border border-white/10 p-4 bg-bg-mid/40">
        ...
        <div className="text-sm opacity-80 grid gap-1">
          <div>ID: <span className="opacity-70">{journeyId}</span></div>
        ...
        <div className="mt-4 grid gap-2 text-sm">
          <label className="grid gap-1">
            ...
            <select className="bg-bg-mid/50 border border-white/10 rounded p-2" value={persona} onChange={e=>setPersona(e.target.value)}>
          ...
          <label className="grid gap-1">
            ...
            <select className="bg-bg-mid/50 border border-white/10 rounded p-2" value={mode} onChange={e=>setMode(e.target.value as any)}>
          ...
          <label className="grid gap-1">
            ...
            <select className="bg-bg-mid/50 border border-white/10 rounded p-2" value={language} onChange={e=>setLanguage(e.target.value as any)}>
          ...
          <label className="grid gap-1">
            ...
            <select className="bg-bg-mid/50 border border-white/10 rounded p-2" value={phaseId} onChange={e=>setPhaseId(e.target.value)}>
          ...
          <label className="grid gap-1">
            ...
            <select className="bg-bg-mid/50 border border-white/10 rounded p-2" value={trackId} onChange={e=>setTrackId(e.target.value)}>
              <option value="builder">Builder</option>
              <option value="investor">Investor</option>
              <option value="student">Student</option>
            </select>
        ...
        <a className="mt-4 inline-block text-xs underline" href={rightLogHref}>View logs</a>
      </aside>
      ...
      <main className="col-span-12 md:col-span-6 rounded-xl border border-white/10 p-4 bg-bg-mid/40 min-h-[60vh]">
        ...
        <div className="grid gap-2">
          ...
          <div className="flex gap-2">
            <button disabled={loading} onClick={()=>runStep()} className="rounded px-3 py-1.5 bg-white/10 border border-white/10 text-sm">
              {loading ? 'Running…' : 'Run step'}
            </button>
            {resp?.ui_blocks?.length ? (
              <button disabled={loading} onClick={()=>{
                const asb = (resp.ui_blocks as any[]).find((b:any)=> b.kind==='action_suggestions_block')
      ...
      <aside className="col-span-12 md:col-span-3 rounded-xl border border-white/10 p-4 bg-bg-mid/40">
        ...
        <div className="text-sm opacity-80">
          {resp?.agent_actions ? (
            <pre className="text-xs whitespace-pre-wrap bg-black/30 p-2 rounded max-h-[300px] overflow-auto">{JSON.stringify(resp.agent_actions, null, 2)}</pre>
          ) : (
        ...
        <div className="text-sm opacity-80">
          {resp?.ui_blocks?.filter((b:any)=> b.kind==='resource_block')?.length ? (
            <UIBlocksRenderer blocks={resp.ui_blocks.filter((b:any)=> b.kind==='resource_block')} />
          ) : (

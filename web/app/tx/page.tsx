...
export default function TxPage(){
  ...
  async function prepareAndSend(){
    ...
    if(!payer){ setStatus('Connectez un wallet'); return }
    ...
    if(!res.ok || !json || !json.tx){ setStatus('Erreur préparation TX'); return }
    ...
    if(!wallet.signTransaction){ setStatus('Wallet ne supporte pas signTransaction'); return }
  ...
  return (
    <main className="min-h-screen p-8 lg:p-12">
      <h1 className="text-3xl font-medium mb-6" data-testid="tx-heading">Préparer une transaction</h1>
      <div className="rounded-2xl p-6 bg-bg-mid/60 border border-white/10 shadow-default">
        <div className="flex flex-col gap-4">
          <label className="flex flex-col">
            ...
            <input className="text-black px-3 py-2 rounded" value={to} onChange={e=>setTo(e.target.value)} placeholder="Adresse Solana" />
          </label>
          <label className="flex flex-col">
            <span className="opacity-75">Lamports</span>
            <input className="text-black px-3 py-2 rounded" value={lamports} onChange={e=>setLamports(e.target.value)} />
          </label>
          <button className="btn btn-primary w-fit" data-testid="tx-submit" onClick={prepareAndSend}>Préparer et envoyer</button>
          {!isConnected && <p className="opacity-90" data-testid="tx-wallet-cta">Connectez un wallet</p>}
          {status && <p className="opacity-90" data-testid="status">{status}</p>}
        </div>
      </div>
    </main>
  )

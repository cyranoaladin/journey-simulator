...
export async function createSiwsChallenge(addressHint?: string): Promise<SiwsChallenge> {
  ...
  await redis.set(`siws:${id}`, JSON.stringify(challenge), 'EX', CHALLENGE_TTL_SECONDS)
  return challenge
}

export async function getSiwsChallenge(id: string): Promise<SiwsChallenge | null> {

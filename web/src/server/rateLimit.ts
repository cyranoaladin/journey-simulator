...
function keyFrom(req: Request): string {
  ...
  const h = (req as any).headers?.get ? (req as any).headers.get('x-forwarded-for') || (req as any).headers.get('cf-connecting-ip') || '' : ''
  const ua = (req as any).headers?.get ? (req as any).headers.get('user-agent') || '' : ''
  return h || `ua:${ua}`
}

export function rateLimit(req: Request, opts?: { maxPerMinute?: number }): { allowed: boolean; remaining: number; resetMs: number }{
  const max = Math.max(1, Math.min(600, Number(opts?.maxPerMinute ?? process.env.RATE_LIMIT_PER_MINUTE ?? 60)))
  const now = Date.now()
  const minute = 60_000
  const k = keyFrom(req)
  ...
  cur.count++
  return { allowed: true, remaining: Math.max(0, max - cur.count), resetMs: Math.max(0, cur.resetAt - now) }
}

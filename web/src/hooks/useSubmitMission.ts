...
export const useSubmitMission = () => {
  ...
  // Import des actions du store global (pour débloquer la suite)
  const { addXP, addMFAI, unlockNextPhase } = useJourneyStore()

  const submitDeliverable = useCallback(
    async (missionId: string, content: string) => {
      ...
      try {
        ...
        // Dans un vrai backend, ces étapes pourraient venir d'un WebSocket/SSE
        for (let i = 0; i < AGENT_STEPS.length; i++) {
          setCurrentStep(AGENT_STEPS[i])
          setProgress(((i + 1) / AGENT_STEPS.length) * 100)

          // Délai artificiel pour que l'utilisateur "sente" le travail (800ms à 1.5s)
          await new Promise((r) => setTimeout(r, 800 + Math.random() * 500))
        }

        // 3. Appel API (Ici Mocké pour le test immédiat, à remplacer par fetch('/api/zyno/evaluate'))
        // C'est ici que Zyno évalue le Markdown
        ...
        if (result.isPassing) {
          // A. Update visuel immédiat
          triggerSuccessEffects()

          // B. Update du Store Global (State durable)
          addXP(60) // Valeur de la mission
          addMFAI(6.0)
          unlockNextPhase(missionId)
        } else {
          // Feedback constructif en cas d'échec
  ...
  return {
    ...
    analysis: {
      strengths: hasKeywords
        ? [
            'Excellent identification of core Web3 pillars.',
            'Clear mapping of Legacy vs Decentralized.',
          ]

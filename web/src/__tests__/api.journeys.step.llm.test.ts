...
const sampleOut = {
  metadata: { persona_id: 'p', journey_track: 't', phase_id: 'ph', language: 'fr', mode: 'builder', tone: 'pedagogical', title: 'Step' },
...
jest.mock('../../src/server/zyno', () => ({
  callZynoStep: jest.fn(async () => ({ out: sampleOut, meta: { usage: { input_tokens: 10, output_tokens: 5, total_tokens: 15 } } })),
}))
...
describe('API /api/journeys/[id]/step LLM branch', () => {
  it('returns 200 with ?llm=1', async () => {
    ...
    const req = { json: async () => ({ phaseId: 'ph', trackId: 't', language: 'fr', journeyState: {} }), url: url.toString() } as any

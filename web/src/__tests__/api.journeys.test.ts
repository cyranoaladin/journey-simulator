...
jest.mock('../server/db', () => ({ prisma: { journey: { findMany: jest.fn(async () => ([])) , create: jest.fn(async (d:any)=>({ id:'j1', ...d.data })) }, user: { upsert: jest.fn(async (d:any)=>({ id:'u1', email:d.where.email })) } } }))

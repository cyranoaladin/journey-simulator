...
const sampleEval = {
  metadata: { persona_id: 'demo', journey_track: 'builder', phase_id: 'learn', language: 'fr', mode: 'builder', tone: 'pedagogical', title: 'Eval' },
  ui_blocks: [
    { kind: 'evaluation_block', id: 'e1', title: 'Feedback', global_score: 84, max_score: 100, feedback: 'Bien', axes: [
...
describe('API /api/journeys/[id]/submit', () => {
  ...
  it('returns demo evaluation when DEMO_MODE=true', async () => {
    ...
    const req = { json: async () => ({ missionId: 'm1', inputType: 'text', submission: 'mon livrable', language: 'fr' }), url: url.toString() } as any
  ...
  it('forces LLM path with ?llm=1 and uses evaluator mock', async () => {
    ...
    const req = { json: async () => ({ missionId: 'm1', inputType: 'text', submission: 'livrable', language: 'fr' }), url: url.toString() } as any
    const res = await POST(req, { params: { id: 'abc' } } as any)
    expect(res.status).toBe(200)
    const json = await (res as NextResponse).json()

...
- **A Node.js/Express + Postgres/Redis API** (`mf-back/`) that manages authentication, progress, analytics, and security.
...
|
|-- REST API (mf-back/)
|   |-- JWT authentication (middleware/auth.js, services/authService.js)
|   |-- Prisma Models (prisma/schema.prisma)
|   |-- REST routes (routes/*.js)
|   `-- Logging, CORS, analytics
|
|-- PostgreSQL (Primary Data)
|   `-- Tables: User, Journey, Artifact, AgentRun
|
`-- Redis (Cache/Session)
    `-- Keys: session:*, throttle:*, cache:*
```
...
| **Backend** | Node 18, Express 4, Prisma 5, JWT, bcrypt, nodemon |
...
- Loads `dotenv` and connects to PostgreSQL/Redis via Prisma.
...
- Fields: `id`, `email`, `password_hash`, `wallet_address`, `persona`, `role`.
- Progress tracking: `xp_total`, `level`, `completed_phases`, `tokens_balance`.
- Relations: `journeys`, `artifacts`, `agent_runs`.

#### `Journey` (PostgreSQL)

- Stores per-user journey state: `phase_status` (JSON), `current_phase`, `completed_at`.

#### `Artifact` & `AgentRun`

- Tracks produced deliverables (PDFs, JSON) and AI agent execution history.
...
1. `npm install` restores Express/Prisma dependencies.
2. `npm run dev` fires up nodemon against `./bin/www`.
3. Ensure PostgreSQL and Redis containers are running (`docker-compose up -d`).
...
### 9.2 Backend (`prisma/schema.prisma`)

- `User` is the source of truth for progress, tokens, and identity.
- `Journey` stores journey metadata per player, including `phases_status` (JSON) for analytics.
- `Artifact` references generated content tied to a journey.

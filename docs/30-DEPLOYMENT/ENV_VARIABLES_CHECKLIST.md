...
- **`DATABASE_URL`** : URL de connexion PostgreSQL (Prisma)
  - Exemple : `postgresql://postgres:password@localhost:5432/journey`
  - **⚠️ CRITIQUE** : Requis pour la persistance relationnelle

- **`REDIS_URL`** : URL de connexion Redis
  - Exemple : `redis://localhost:6379`
  - **⚠️ CRITIQUE** : Requis pour le cache et les files d'attente
...
- [ ] `DATABASE_URL` pointe vers la base de production (Postgres)
- [ ] `REDIS_URL` pointe vers le service Redis interne
...
node -e "
const required = ['OPENAI_API_KEY', 'RAG_SEARCH_URL', 'RAG_API_KEY', 'DATABASE_URL', 'REDIS_URL', 'JWT_SECRET', 'ADMIN_API_KEY'];
required.forEach(v => {

...
### Serveurs inclus
- **`filesystem_ro`** : accès fichier en allowlist (repo + dossiers utiles).
- **`git`** : historique/diff/blame.
- **`postgres_ro`** : inspection Postgres (Prisma) en lecture.
- **`fetch`** : ingestion de doc web (Solana/Metaplex/SIWS, etc.).
...
- DSN par défaut dans `mcp.json` :
  - `postgresql://prisma:prisma@127.0.0.1:5435/prisma`
...
- `postgresql://prisma:prisma@127.0.0.1:5433/prisma`

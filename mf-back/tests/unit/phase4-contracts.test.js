...
describe('Phase 4: Contract Locking (Unit Tests)', () => {
    ...
    test('Action G4: Timeline sanitizer removes secrets', () => {
        const timeline = [
            {
                agent: 'TestAgent',
                prompt: 'Use access token Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0.SIGNATURE_REMOVED_FOR_TEST to fetch data',
                summary: 'Call API with sk-test-dummy-key-for-sanitization-check-must-be-long key'
            }
        ];
        ...
        expect(sanitized[0].summary).not.toContain('sk-test-');
    });

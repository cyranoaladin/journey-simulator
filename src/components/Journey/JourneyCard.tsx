...
const JourneyCard: React.FC<JourneyCardProps> = ({ persona }) => {
  const { setSelectedPersona, userProgress, loadUserProgress } =
    useJourneyStore();
  ...
  const progressPercentage = hasStarted
    ? Math.min(
        (userProgress.completedPhases.length / persona.phases.length) * 100,
        100,
      )
    : 0;
  ...
  return (
    ...
    >
      <div className="text-center">
        <div
          className={`w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-r ${persona.color} flex items-center justify-center text-2xl`}
        >
          {getPersonaIcon()}
        </div>
        ...
        <div className="flex justify-between items-center mb-3 text-xs opacity-70">
          ...
          {hasStarted && (
            <span>
              {userProgress.completedPhases.length}/{persona.phases.length}{" "}
              completed
            </span>
          )}
        </div>

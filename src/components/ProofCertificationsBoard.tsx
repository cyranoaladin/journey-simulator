...
const ProofCertificationsBoard: React.FC<ProofCertificationsProps> = ({
  className = "",
}) => {
  ...
  const getCertifications = () => {
    ...
    for (
      let i = userProgress.completedPhases.length;
      i < selectedPersona.phases.length;
      i++
    ) {
    ...
    <div className={`${className}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm opacity-70">
          {userProgress.nfts.length} / {certifications.length}
        </span>
      </div>
      ...
      {certifications.length === 0 ? (
        <div className="text-center py-8 bg-white/5 rounded-xl border border-white/10">
          ...
          <p className="text-sm opacity-60">
            Complete journey phases to earn Proof-of-Skill™ NFTs
          </p>
        </div>
      ) : (
          ...
          {certifications.map((certification, index) => {
            ...
            return (
              <motion.div
                ...
                onClick={() =>
                  !isLocked && handleCertificationClick(certification)
                }
              ...
              >
                <div className="flex items-center space-x-3">
                  {/* NFT Icon */}
                  <div
                    className={`w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 ${isLocked ? "bg-white/10" : `bg-gradient-to-br ${personaStyle.bgGradient}`}`}
                  >
                    {isLocked ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4
                        className={`font-semibold text-sm truncate ${isLocked ? "opacity-50" : ""}`}
                      >
                        {certification.name}
                      </h4>
                      <span
                        className={`ml-2 px-2 py-0.5 rounded-full text-xs font-semibold ${getRarityColor(certification.rarity)}`}
                      >
                    ...
                    <div className="flex items-center justify-between">
                      <span
                        className={`text-xs ${isLocked ? "opacity-50" : "opacity-80"}`}
                      >
                        Proof-of-{proofType}™
                      </span>
      ...
      <AnimatePresence>
        {showProofModal && selectedCertification && (
          <NFTProofModal
            ...
            xpEarned={
              (selectedCertification.attributes.find(
                (a) => a.trait_type === "XP Earned",
              )?.value as number) || 0
            }
            phase={
              (selectedCertification.attributes.find(
                (a) => a.trait_type === "Phase",
              )?.value as string) || ""
            }
            phaseNumber={
              userProgress.completedPhases.findIndex(
                (p) =>
                  selectedPersona?.phases[p].nftReward ===
                  selectedCertification.name,
              ) + 1
            }
            completionDate={
              (selectedCertification.attributes.find(
                (a) => a.trait_type === "Completion Date",
              )?.value as string) || new Date().toLocaleDateString()
            }

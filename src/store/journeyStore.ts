  ...
  persist(
    (set, get) => ({
      ...
      completePhase: async (phaseIndex) => {
        ...
        const updatedPhases = [
          ...state.userProgress.completedPhases,
          phaseIndex,
        ];
      ...
      updateStaking: (amount) =>
        set((state) => ({
          userProgress: {
            ...
            votingPower:
              state.userProgress.votingPower + Math.floor(amount * 2), // 2 voting power per staked MFAI
          },
        })),

      updateVotingPower: (newPower) =>
        set((state) => ({
          userProgress: {
            ...state.userProgress,
      ...
      updateWalletConnection: (connected, address) =>
        set((state) => {
          ...
          return {
            userProgress: {
              ...
              mfaiTokens:
                connected && state.userProgress.mfaiTokens === 0
                  ? 10
                  : state.userProgress.mfaiTokens,
            },
          };
        }),
      ...
      completeMission: () =>
        set((state) => {
          ...
          const mfaiReward = 25;
          const nftReward =
            Math.random() > 0.5 ? ["Mission Completion NFT"] : [];
          ...
          return {
            userProgress: {
              ...
              votingPower:
                state.userProgress.votingPower + Math.floor(xpReward / 10),
            },
          };
        }),
        ...
        try {
          ...
          if (response.success) {
            ...
            const mappedProgress: UserProgress = {
              ...
              nfts:
                progress.nft_certificates?.map(
                  (cert: any) => `Phase ${cert.phase} NFT`,
                ) || [],
              passLevel:
                progress.subscription === "free plan"
                  ? "Free"
                  : progress.subscription === "gold"
                    ? "Gold"
                    : progress.subscription === "platinum"
                      ? "Platinum"
                      : "Diamond",
              ...
              completedPhases: Array.from(
                { length: progress.completed_phases || 0 },
                (_, i) => i,
              ),
              currentPersona: undefined,
